...
<video src="https://github.com/SeasonCake/openclaw-discord-bot/raw/main/assets/videos/demo.mp4" controls width="100%" muted>
  Your browser does not support HTML5 video. Watch on <a href="https://youtu.be/4KZbtfR2rOY">YouTube</a> instead.
</video>

**▶ Also on YouTube:** [https://youtu.be/4KZbtfR2rOY](https://youtu.be/4KZbtfR2rOY) (unlisted)
...
### The core idea: one bot, two skills, zero mode switching

The screenshot below shows a **full, unedited chat flow** — the same user (`Qilindage`) talks to the same bot (`@openclaw_bidking`). The user never selects a mode, never says "switch to X skill". They just drop a CSV and say "analyze it", then later say "open a game". The OpenClaw framework reads the intent and routes to the right skill automatically.

<p align="center">
  <img src="./assets/screenshots/04-full-chat-flow.png" alt="Full Discord chat flow showing natural-language intent routing between two skills" width="560" />
</p>

Sample EDA chart outputs (generated by `csv_analyzer` on different datasets): [`eda-sample-1`](./assets/screenshots/eda-sample-1.webp) · [`eda-sample-2`](./assets/screenshots/eda-sample-2.webp)
